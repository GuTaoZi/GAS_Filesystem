...
static int block_to_path(struct inode *inode, long block, int offsets[DEPTH])
{
    ...
    if (block < 0)
    {
        pr_debug("gas: block_to_path: block %ld < 0 on dev %s\n", block, bdevname(sb->s_bdev, b));
    }
    else if (block >= GAS_SB(sb)->s_nblocks)
    {
        if (printk_ratelimit())
            pr_debug("gas: block_to_path: "
...
static int alloc_branch(struct inode *inode, int num, int *offsets, Indirect *branch)
{
    ...
    int i;
    int err;
    int parent = gas_new_block(inode, &err);
    ...
    if (parent)
        for (n = 1; n < num; n++)
        {
            ...
            int nr = gas_new_block(inode, &err);
            if (!nr)
                break;
    ...
    for (i = 1; i < n; i++)
        bforget(branch[i].bh);
    for (i = 0; i < n; i++)
        gas_free_block(inode, block_to_cpu(branch[i].key));
    return -ENOSPC;
}

static inline int splice_branch(struct inode *inode, Indirect chain[DEPTH], Indirect *where, int num)
{
    int i;
...
changed:
    ...
    for (i = 1; i < num; i++)
        bforget(where[i].bh);
    for (i = 0; i < num; i++)
        gas_free_block(inode, block_to_cpu(where[i].key));
    return -EAGAIN;
}
...
static Indirect *find_shared(struct inode *inode, int depth, int offsets[DEPTH], Indirect chain[DEPTH], block_t *top)
{
    Indirect *partial, *p;
    int k, err;
...
void gas_truncate_inode(struct inode *inode)
{
    ...
    while (partial > chain)
    {
        free_branches(inode, partial->p + 1, block_end(partial->bh), (chain + n - 1) - partial);
        mark_buffer_dirty_inode(partial->bh, inode);
        brelse(partial->bh);
        partial--;
    }
