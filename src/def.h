...
static struct file_system_type gas_fs_type = {
    .owner = THIS_MODULE, .name = "sfs", .mount = gas_mount, .kill_sb = kill_block_super, .fs_flags = FS_REQUIRES_DEV};
...
static struct inode_operations gas_inode_ops = {
    .create = gas_create, .lookup = gas_lookup, .mkdir = gas_mkdir, .permission = gas_permission};
